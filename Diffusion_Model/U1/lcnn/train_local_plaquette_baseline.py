...
def angles_to_cossin(angles: np.ndarray) -> np.ndarray:
    x = angles[:, 0]
    y = angles[:, 1]
    return np.stack([np.cos(x), np.sin(x), np.cos(y), np.sin(y)], axis=1)
...
def random_gauge_angles(batch: int, L: int, rng: np.random.Generator) -> np.ndarray:
    return rng.uniform(-math.pi, math.pi, size=(batch, L, L))


def gauge_transform_angles(angles: np.ndarray, alpha: np.ndarray) -> np.ndarray:
    transformed = np.empty_like(angles)
    transformed[:, 0] = wrap_angle(angles[:, 0] + alpha - np.roll(alpha, shift=-1, axis=1))
    transformed[:, 1] = wrap_angle(angles[:, 1] + alpha - np.roll(alpha, shift=-1, axis=2))
    return transformed
...
class PlainPlaquetteCNN(nn.Module):
...
def gauge_robustness_mae(model: nn.Module, dataset: U1PlaquetteDataset, device: torch.device, seed: int) -> float:
    ...
    angles = cossin_to_angles(features.astype(np.float64))
    alpha = random_gauge_angles(batch=angles.shape[0], L=angles.shape[-1], rng=rng)
    transformed_angles = gauge_transform_angles(angles, alpha)
    transformed_features = angles_to_cossin(transformed_angles).astype(np.float32)

    with torch.no_grad():
        original_pred = model(dataset.features.to(device)).cpu()
        transformed_pred = model(torch.from_numpy(transformed_features).to(device)).cpu()
    return float(torch.mean(torch.abs(original_pred - transformed_pred)).item())
...
def train(args: argparse.Namespace) -> dict:
    ...
    model = PlainPlaquetteCNN(hidden_channels=args.hidden_channels).to(device)
    ...
    checkpoint_path = os.path.join(args.output_dir, "plain_cnn_local_plaquette.pt")
    metrics_path = os.path.join(args.output_dir, "plain_cnn_local_plaquette_metrics.json")
    torch.save(
        {
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(description="Train a plain CNN baseline on local plaquette regression.")
    parser.add_argument(
        "--train-npz",
        type=str,
        default="/Users/wangkehe/Git_repository/Diffusion_Model/U1/preprocess/output/u1_train.npz",
    )
    parser.add_argument(
        "--val-npz",
        type=str,
        default="/Users/wangkehe/Git_repository/Diffusion_Model/U1/preprocess/output/u1_val.npz",
    )
    parser.add_argument(
        "--test-npz",
        type=str,
        default="/Users/wangkehe/Git_repository/Diffusion_Model/U1/preprocess/output/u1_test.npz",
    )
    parser.add_argument(
        "--output-dir",
        type=str,
        default="/Users/wangkehe/Git_repository/Diffusion_Model/U1/lcnn/output",
    )
...
def main() -> None:
    args = parse_args()
    result = train(args)
    print("[DONE] Plain CNN local plaquette training completed")
